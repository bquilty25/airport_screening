...
write_date_stamp <- function() {
  sprintf(
  "Last built at %s by B. Quilty, S. Clifford, S. Flasche, R. Eggo and other members of CMMID at LSHTM",
  format(
    Sys.time(),
    "%d %b %Y at %H:%M:%S"
  )
) %>%
  write(file = "inst/info/date_stamp.md")
